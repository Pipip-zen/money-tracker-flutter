...
class _StatisticsScreenState extends ConsumerState<StatisticsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final sixMonthsAgo = DateTime(
      _currentMonth.year,
      _currentMonth.month - 5,
      1,
    );

    final currentMonthAsync = ref.watch(
      transactionsByDateRangeProvider((from: firstDay, to: lastDay)),
    );

    final sixMonthsAsync = ref.watch(
      transactionsByDateRangeProvider((from: sixMonthsAgo, to: lastDay)),
    );
  ...
  Widget _buildBarChart(
    ...
    for (var tx in transactions) {
      int index =
          (tx.date.year - startMonth.year) * 12 +
          (tx.date.month - startMonth.month);
      if (index >= 0 && index < 6) {
        if (tx.type == 'income') {
